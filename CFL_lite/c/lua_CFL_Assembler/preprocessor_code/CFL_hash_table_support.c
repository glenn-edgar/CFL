...
static int fnv_hash_CFL(const void *data, int len) {
...
static int fnv_hash_string_CFL(const char *data) {
  int len;
  len = strlen(data);
...
static int lua_fnv_hash_string_CFL(lua_State *L ){
   const char* string_name = luaL_checkstring(L, 1);  
    int hash = fnv_hash_string_CFL(string_name);
    lua_pushinteger(L, hash);
    return 1;
}

 static const luaL_Reg mylib_functions[] = {
   {"CFL_hash", lua_fnv_hash_string_CFL},
    {NULL, NULL}
};
