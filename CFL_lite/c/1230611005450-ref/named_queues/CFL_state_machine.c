...
void Asm_sync_events_CFL(void *input,unsigned short number_of_events, short *event_indexes){
   
   if(number_of_events == 0){
      ASSERT_PRINT("number_of_events == 0\n","");
   }
    syn_events_t *syn_events = (syn_events_t *)Allocate_once_malloc_CFL(input, sizeof(syn_events_t));
    syn_events->number_of_events = number_of_events;
    syn_events->event_indexes = (unsigned short *)Allocate_once_malloc_CFL(input, sizeof(unsigned short) * number_of_events);
    for(unsigned i = 0; i < number_of_events; i++){
        syn_events->event_indexes[i] = event_indexes[i];
    }
    Asm_store_column_element_CFL(input, "SYNC_EVENTS",NULL, syn_events);
...
static int sync_events(void *input, void *aux_fn, void *params, Event_data_CFL_t *event_data){
    ...
    if(match_event(syn_events->number_of_events,syn_events->event_indexes,event_data->event_index) == true){
        return DISABLE_CFL; // column element is disabled            
    }
    // unmatched  event
    return HALT_CFL; // block until matched event
}
...
void Asm_transfer_events_to_sm_state_CFL(void *input,const const char *transfer_sm,const char *state, 
    ...
    short state_id = Find_Name_CFL(sm_dict->sm_names, state);
    if (state_id < 0)
    {
        ASSERT_PRINT_F("state %s not found\n", state);
    }


    transfer_to_sm_state_t *transfer_to_sm_state = (transfer_to_sm_state_t *)Allocate_once_malloc_CFL(input, sizeof(transfer_to_sm_state_t));
    transfer_to_sm_state->sm_id_transfer = sm_id_transfer;
    transfer_to_sm_state->state = state_id;
    transfer_to_sm_state->number_of_events = number_of_events;
    if(number_of_events == 0){
        transfer_to_sm_state->event_indexes = NULL;
    }else{
        transfer_to_sm_state->event_indexes = (unsigned short *)Allocate_once_malloc_CFL(input, sizeof(unsigned short) * number_of_events);
        for(unsigned i = 0; i < number_of_events; i++){
            transfer_to_sm_state->event_indexes[i] = event_indexes[i];
        }
    }
    
    Asm_store_column_element_CFL(input, "TRANSFER_EVENTS_TO_SM_STATE",NULL, transfer_to_sm_state);
...
void Change_state_CFL(void *input, short sm_id, short new_state_id)
{
    ...
    Reset_named_event_queue_CFL(handle, sm_control->queue_ids[sm_control->current_state]);
