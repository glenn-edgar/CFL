...
Floatmap_CFL_t* get_floatmap_control_CFL(const void *input, unsigned buffer_number){
   ...
   if(buffer_number >= handle->number_of_floatmaps){
     ASSERT_PRINT_F(input,"float_map_number is out of range got %d, max %d\n", buffer_number, handle->number_of_registermaps);
   }
...
void floatmap_set_value_CFL(const void *input,Floatmap_CFL_t* bmp, unsigned index, float value) {
    if (index >= bmp->length) {
        ASSERT_PRINT_F(input,"floatmap_set_value_CFL: index %d is out of range\n", index);
        return;
    }
...
float floatmap_get_value_CFL(const void *input,Floatmap_CFL_t* bmp, unsigned index) {
    if (index >= bmp->length) {
        ASSERT_PRINT_F(input,"floatmap_get_value_CFL: index %d is out of range\n", index);
        return 0;
    }
...
void floatmap_set_all_CFL(const void *input,Floatmap_CFL_t* bmp, float value) {
   for(unsigned i=0; i<bmp->length; i++){
       floatmap_set_value_CFL(input,bmp, i, value);
   }
}
