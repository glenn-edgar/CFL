...
static int16_t pop_last_instruction(const void *input,s_register_working_control_CFL_t *working_control);
...
static int16_t pop_last_instruction(const void *input,s_register_working_control_CFL_t *working_control)
{
        ...
        if (stack_number != 1)
        {
                ASSERT_PRINT_F(input,"s_register_engine: parameter stack size is not equal to 1 %d\n", stack_number);
        }
...
static unsigned verify_expression_start(const void *input, s_register_working_control_CFL_t *working_control, s_register_operator_CFL_t operator)
{
        ...
        register_push_op_stack_CFL(input,working_control->stack_control, &s_op);
...
static unsigned evaluate_expression(const void *input, s_register_working_control_CFL_t *working_control, s_register_operator_CFL_t operator)
{
        ...
        case S_REGISTER_VALUE_CFL:
                ...
                register_push_parameter_stack_CFL(input,working_control->stack_control, &parameter);
                return_value = EVALUATE_EXPRESSION;
                break;
        case S_REGISTER_OPERATOR_END_CFL:;
                s_register_operator_type_CFL_t *working_instruction = register_pop_op_stack_CFL(input,working_control->stack_control);
                ...
                break;
        default:
                ASSERT_PRINT_F(input,"s_reg_engine: unknown operator type %d\n", operator_type);
                break;
        }
