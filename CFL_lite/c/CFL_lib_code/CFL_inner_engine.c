...
static inline void change_column_state(const void *input, unsigned column_index)
{
  ...
  if (start_state < 0)
  {
    ASSERT_PRINT_INT(handle,"invalid column state machine start state", start_state);
  }
  int end_state = column_ROM->end_state;
  if (end_state < 0)
  {
    ASSERT_PRINT_INT(handle,"invalid column state machine end state", end_state);
  }
...
static inline void validate_return_code(const Handle_CFL_t *handle, int return_code)
{
  ...
  case HALT_AND_DISABLE_CFL:
    return;
  default:

    ASSERT_PRINT_INT(handle,"invalid return code", return_code);
  }
}
                                        ...
                                        Event_data_CFL_t *event_data)
{
  ...
  for (unsigned i = 0; i < column->number; i++)
  {
    ...
    validate_return_code(handle,return_code);
                              ...
                              Event_data_CFL_t *event_data)
{
  ...
  const Handle_CFL_t *handle = (const Handle_CFL_t *)input;
    ...
    const Column_ROM_CFL_t *column = handle->column_rom_data + column_index;
    unsigned column_id_ref = handle->engine_control->current_column_index ;
    if ((handle->column_flags[column_index] & COLUMN_ACTIVE) == 0)
    {
    ...
    if (column->queue_number >= 0)
    {
      ...
      result = process_column_named_events(handle, column_index, event_data);
    }
    if (result == true)
    {
  ...
  handle->engine_control->current_column_index =  column_id_ref;  
  return true;
...
void enable_column_CFL(const void *input, unsigned column_index)
{
  ...
  handle->column_flags[column_index] &= ~(WATCH_DOG_ACTIVE);
...
void initialize_columns_CFL(const void *input)
{
  ...
  for (unsigned i = 0; i < handle->number_of_columns; i++)
  {
    ...
    if (column[i].auto_start == true)
    {
      ...
      handle->column_state[i]  &= ~WATCH_DOG_ACTIVE;
    ...
    else
    {
      ...
      handle->column_flags[i]  &= ~(COLUMN_ACTIVE | WATCH_DOG_ACTIVE);
    }
  }
}
...
void change_local_column_state_CFL(const void *input, unsigned char new_state)
{
  ...
  {
    ASSERT_PRINT(input,"Column is not a column state machine", "");
  }
  unsigned state_number = column_ROM->end_state - column_ROM->start_state;
  if (new_state > state_number)
  {

    ASSERT_PRINT_INT(input,"New state is out of range", new_state);
  }
  else
  {
...
void attach_watch_dog_handler_CFL(const void *input, const Watch_dog_struct_CFL_t *watch_dog_struct)
{
  ...
  if ((handle->column_flags[column_index] & WATCH_DOG_ACTIVE) != 0)
  {
    ASSERT_PRINT_INT(input,"column has watch dog active", column_index);
  }
...
void store_column_data_CFL(const void *input, unsigned short column_id, void *column_data)
{
  ...
  {
    ASSERT_PRINT_INT(input,"invalid column id", column_id);
  }
...
void store_current_column_data_CFL(const void *input,  void *column_data)
{
  ...
  Handle_CFL_t *handle = (Handle_CFL_t *)input;
  Engine_control_CFL_t *engine_control = handle->engine_control;
  unsigned column_id = engine_control->current_column_index;
  if (column_id >= handle->number_of_columns)
  {
    ASSERT_PRINT_INT(input,"invalid column id", column_id);
  }
...
void *retreive_column_data_CFL(const void *input, unsigned short column_id)
{
  ...
  return handle->column_local_data[column_id];
}

bool get_column_state_CFL(const void *input, unsigned short column_index){
  const Handle_CFL_t *handle = (const Handle_CFL_t *)input;
  if(column_index >= handle->number_of_columns){
    ASSERT_PRINT_INT(input,"invalid column index", column_index);
  }
  return (handle->column_flags[column_index] & COLUMN_ACTIVE);
