...
/*
   Windows subsystem has a problem with nanosleep() function.
   had to pool for time for the delay_ms to work.
                               ...
                               bool init_flag)
{
    ...
    if (init_flag == true)
    {
        ...
        timer_control->month = timeInfo->tm_mon+1; // tm_mon months are zero based
    ...
    else
    {
        ...
        if (timer_control->month != timeInfo->tm_mon+1) // tm_mon months are zero based
        {
            event_data.event_index = MONTH_TICK_CFL;
            event_data.params = &timeInfo->tm_mon+1;
            event_data.malloc_flag = false;
            queue_global_event_CFL(handle, &event_data);
        }
        timer_control->month = timeInfo->tm_mon+1;
