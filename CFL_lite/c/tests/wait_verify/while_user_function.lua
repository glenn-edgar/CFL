set_h_file("while_user_function.h")

local entry_point = "while_user_function_handle"
...
Store_one_shot_function("WAIT_EVENT_RESET_C",'wait_event_reset',wait_event_reset_fn_code,wait_event_reset_fn_header)
...
Store_one_shot_function("WAIT_EVENT_TERMINATE_C",'wait_event_terminate',wait_event_terminate_fn_code,wait_event_terminate_fn_header)
...
define_column("termination_engine",true,0)
...
define_column("generate_event",true,0)
...
define_column("user_function",true,0)
...
define_column("wait_reset",true,0)
   Log_msg("this demonstrates the reset feature of the wait opcode")
   Wait("FALSE",5000, false, "WAIT_EVENT_RESET_C", 'wait_event_reset_message')
   Log_msg("10 events have been received")
   Log_msg("terminating column")
   terminate_column()
end_column()

define_column("wait_terminate",true,0)
   Log_msg("this demonstrates the terminate feature of the wait opcode")
   Wait("FALSE",5000, true, "WAIT_EVENT_TERMINATE_C", 'wait_event_terminate_message')
