...
local function dump_ram_data_structures()
   local number = 0
   local count  = 0
   for queue_name,queue_data in pairs(queue_names) do
      count = count + 1
      number = number + queue_data[2]
   end
   write_output("\n\n//----------RAM data structures for event queues ----\n\n")
   format_string = "Event_data_CFL_t event_data_array[%d];\n"
   local message = string.format(format_string,number)
   
   write_output(message)
   format_string = "Event_control_RAM_t event_control_ram[%d];\n"
...
local function dump_rom_data_structures()
    ...
    for queue_name,queue_data in pairs(queue_names) do
        format_string = "const Event_control_ROM_t event_control_rom_%s = { %d, %d };\n"
        size = queue_data[2]
    ...
    format_string ="const Named_event_queue_control_CFL_t queue_control = { %d, {\n"
    local message = string.format(format_string,number)
    write_output(message)
