...
static void match_function(int number_of_records, S_fn_record_CFL_t* records, int* match_index, char* input);
...
static S_record_CFL_t* parse_tokens(void *handle, int fn_number, S_fn_record_CFL_t* s_record, int token_number,  char** string_token_array);



int Evaluate_s_expression_CFL(void* handle, void* s_handle, void* user_data) {
    ...
    int* param_stack = (int*)Private_heap_malloc_CFL(handle, sizeof(int) * s_record_control->token_number);
    // initialize r_stack
    while (token_index < s_record_control->token_number) {
        if (s_record_control->token_array[token_index].fn_flag == false) {
            ...
            *(param_stack + param_stack_index) = s_record_control->token_array[token_index].value;
            ...
            int *param_locations = param_stack + r_stack[r_stack_index].param_stack_index;
...
void Free_s_express_CFL(void* handle, void* s_handle) {
    S_record_Control_CFL_t* s_record_control = (S_record_Control_CFL_t*)s_handle;
...
void* Parse_s_expression_CFL(void* handle, int fn_number, S_fn_record_CFL_t* s_functions, const char* input) {
    char* ref_buffer;
...
static S_record_CFL_t* parse_tokens(void *handle, int fn_number, S_fn_record_CFL_t* s_record, int token_number,  char** string_token_array) {
    ...
    for (int i = 0; i < token_number; i++) {
        ...
        // Check for integer
        else if (sscanf(token, "%d", &token_array[i].value) == 1) {
