...
static float add_function(void *input, void *user_data, int count,
                          float *args) {
  float sum = 0;
...
static float sub_function(void *input, void *user_data, int count,
                          float *args) {
...
static float mul_function(void *input, void *user_data, int count,
                          float *args) {
  float product = 1;
  for (int i = 0; i < count; i++) {
...
static float div_function(void *input, void *user_data, int count,
                          float *args) {
...
static void test_float_array_functions(Handle_config_CFL_t *config) {
  ...
  s_expr_handle =
      Parse_s_float_expression_CFL(input, fn_number, function_map, int_prompt1);
