...
static double add_function(void *input, void *user_data, int count,
                           double *args) {
  double sum = 0;
...
static double sub_function(void *input, void *user_data, int count,
                           double *args) {
...
static double mul_function(void *input, void *user_data, int count,
                           double *args) {
  double product = 1;
  for (int i = 0; i < count; i++) {
    // Printf_CFL("%s %lf args[%d] = %lf \n",__func__,i,args[i]);
...
static double div_function(void *input, void *user_data, int count,
                           double *args) {
...
static void test_double_array_functions(Handle_config_CFL_t *config) {
  ...
  s_expr_handle = Parse_s_double_expression_CFL(input, fn_number, function_map,
                                                int_prompt1);
